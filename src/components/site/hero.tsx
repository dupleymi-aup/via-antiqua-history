    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center relative z-10">
        <div className="container mx-auto max-w-6xl px-4">
          {/* Автор — компактный бейдж на мобильных */}
          ...
          <div className="grid lg:grid-cols-[1fr,auto] gap-8 lg:gap-12 items-start">
            ...
            >
              <h1 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-7xl font-semibold leading-tight tracking-tight">
                <span className="block">Исторический Лабиринт</span>
                <span className="block gold-text mt-2 text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl">
                  От Эллады до Римских Пределов
                </span>
              </h1>

              <p className="mt-4 sm:mt-6 max-w-2xl text-sm sm:text-base md:text-lg text-muted-foreground leading-relaxed font-body">
                ...
                {regionChips.map((chip, i) => {
                  ...
                  return (
                    <Link
                      key={chip.label}
                      href={sectionIds[i]}
                      className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium border border-border bg-card/60 backdrop-blur-sm flex items-center gap-1.5 sm:gap-2 hover:border-primary/40 hover:bg-accent/5 transition-colors"
              ...
              <motion.div
                ...
                className="mt-6 sm:mt-8 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 max-w-xl"
              >
                {stats.map((s, i) => (
              ...
              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <Link
                  href="#map"
                  className="inline-flex items-center justify-center gap-2 h-10 sm:h-11 px-5 sm:px-6 rounded-md border border-border bg-card/60 backdrop-blur-sm hover:bg-accent/10 transition-colors font-medium text-sm sm:text-base"
                >
          ...
          >
            <ChevronDown className="h-5 w-5 sm:h-6 sm:w-6 text-muted-foreground animate-bounce" aria-hidden="true" />
          </motion.div>
        </div>
      </div>
    </section>
...
function StatCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-card/60 backdrop-blur-sm px-2 sm:px-3 py-2 sm:py-3">
      <div className="flex items-center justify-center gap-1 text-primary mb-0.5 sm:mb-1">
        {icon}
        <span className="font-display text-xl sm:text-2xl md:text-3xl font-bold gold-text tabular-nums">
          {animatedValue}
        </span>
      </div>
